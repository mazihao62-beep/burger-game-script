...
if MeleeEvent and PickupEvent and DropEvent then
    remotesReady = true
    print("[Burger v2.11] 远程OK")
else
    warn("[Burger v2.11] 远程缺失")
end
...
local function gN(range)
    ...
    if gf then
        ...
        for _, fn in ipairs({ "Customers", "CustomCustomers", "NPCs" }) do
                ...
                if obj:IsA("Humanoid") and obj.Parent and obj.Parent:IsA("Model") then
                    ...
                    if not seen[m] and not isMe(m) and obj.Health > 0 then
                        ...
                        if mp then
                            ...
                            if d <= rr then
                                table.insert(npcs, { M = m, H = obj, P = mp, D = d })
                            end
                        end
                    end
                end
...
local function mW()
    ...
    CT.KillNPC = t1:Toggle({ Flag = "KillNPC", Title = "自动杀死NPC(爆头)", Value = false,
        Callback = function(v) S.KillNPC = v end })
...
spawn(function()
    ...
    WI:Notify({
        Title = "汉堡 v2.11",
        Content = "远程:" .. (remotesReady and "OK" or "MISS") .. " | 杀NPC | 粉碎 | 收钱",
        Duration = 3,
    ...
    while true do
        if S.AutoMode then
            pcall(function() dK() end); wait(0.3)
            pcall(function() dG() end); wait(0.3)
            pcall(function() dC() end); wait(1)
        else
            if S.KillNPC then pcall(function() dK() end); wait(0.5) end
            if S.GrindBodies then pcall(function() dG() end); wait(0.5) end
