...
if MeleeEvent and PickupEvent and DropEvent then remotesReady = true; print("[v2.12] 远程OK") else warn("[v2.12] 远程缺失") end

for _, g in ipairs(C:GetChildren()) do
    if g:IsA("ScreenGui") then
        local n = g.Name
        if n == "A" or n:find("Burger") or n == "WindUI" then pcall(function() g:Destroy() end) end
    end
end
...
print("[v2.12] WindUI OK")

local S = {
    KillNPC = false, GrindBodies = false, CollectMoney = false, AutoMode = false,
    EspEnabled = false, EspRange = 200, KillRange = 50, AkillDamage = 26,
...
local function mKW(n, l)
    ...
    local ln = n:lower()
    for _, k in ipairs(l) do if ln:find(k, 1, true) then return true end end
    return false
end
...
local function dK()
    ...
    if not hNow or hNow.Health <= 0 then print("[Kill] NPC已死"); return false, "NPC已死" end
...
local function sP()
    ...
    PR = true
    spawn(function() local t = 0; while PR and PC do t = t + 0.03; pcall(function()
...
local function mE(t)
    ...
    local bb = Instance.new("BillboardGui"); bb.Size = UDim2.new(0,200,0,50); bb.MaxDistance = S.EspRange
    bb.AlwaysOnTop = true; bb.StudsOffset = Vector3.new(0,4,0); pcall(function() bb.Parent = t end)
    local tl = Instance.new("TextLabel"); tl.Size = UDim2.new(1,0,1,0); tl.Text = "NPC"
    tl.TextColor3 = Color3.fromRGB(255,40,40); tl.BackgroundTransparency = 0.7
    tl.BackgroundColor3 = Color3.new(0,0,0); tl.TextScaled = true; tl.Font = Enum.Font.SourceSansBold; tl.Parent = bb
    local hl = Instance.new("Highlight"); hl.FillColor = Color3.fromRGB(255,40,40)
    hl.OutlineColor = Color3.fromRGB(255,255,255); hl.FillTransparency = 0.3; hl.OutlineTransparency = 0
    hl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop; pcall(function() hl.Parent = t end)
    EO[t] = {B=bb, H=hl}
end
...
local function mW()
    WN = WI:CreateWindow({
        ...
        Size = UDim2.fromOffset(750, 520), ToggleKey = Enum.KeyCode.RightShift, Folder = "burger-script",
        ...
        OnClose = function()
            ...
            S.AutoMode = false; S.EspEnabled = false; cA()
    ...
    t3:Divider()
    CT.Particles = t3:Toggle({Flag="Particles", Title="粒子背景(全屏)", Value=true, Callback=function(v) S.Particles = v; if v then sP() else xP() end end})
    t3:Toggle({Flag="Acrylic", Title="毛玻璃", Value=true, Callback=function(v) S.Acrylic = v; pcall(function() WI:ToggleAcrylic(v) end) end})
    t3:Toggle({Flag="Transparent", Title="透明背景", Value=false, Callback=function(v) S.Transparent = v; pcall(function() WN:ToggleTransparency(v) end) end})
    ...
    pcall(function()
        ...
        t5:Button({Title="保存", Icon="solar:check-circle-bold", Justify="Center", Color=Color3.fromHex("#305dff"), Callback=function()
            ...
            if c and c:Save() then WI:Notify({Title="已保存", Content="配置已保存", Duration=3, Icon="solar:check-circle-bold"}); pcall(function() ACD:Refresh(CM:AllConfigs()) end) end
        end})
        t5:Space()
        t5:Button({Title="加载", Icon="solar:refresh-circle-bold", Justify="Center", Color=Color3.fromHex("#10C550"), Callback=function()
            ...
            if c and c:Load() then WI:Notify({Title="已加载", Content="配置已加载", Duration=3, Icon="solar:refresh-circle-bold"}) end
        end})
        t5:Space()
        t5:Button({Title="删除", Icon="solar:trash-bin-trash-bold", Justify="Center", Color=Color3.fromHex("#ff3040"), Callback=function()
            if not CM then return end; local c = CM:Config("default")
            if c and c:Delete() then WI:Notify({Title="已删除", Content="配置已删除", Duration=3, Icon="solar:trash-bin-trash-bold"}); pcall(function() ACD:Refresh(CM:AllConfigs()) end) end
        end})
