...
if MeleeEvent and PickupEvent and DropEvent then
    remotesReady = true
    print("[v2.22] remotes OK")
end
for _, g in ipairs(C:GetChildren()) do
    if g:IsA("ScreenGui") and (g.Name == "A" or g.Name:find("Burger") or g.Name == "WindUI") then
        pcall(function() g:Destroy() end)
    end
end
local WI = loadstring(game:HttpGet("https://raw.githubusercontent.com/Footagesus/WindUI/main/dist/main.lua"))()
if not WI then return end
print("[v2.22] WindUI OK")
local S = {KillNPC=false,GrindBodies=false,CollectMoney=false,AutoMode=false,EspEnabled=false,EspRange=200,KillRange=50,AkillDamage=26,Particles=true,Acrylic=true,Transparent=false,ParticleColor=Color3.fromRGB(80,170,255)}
...
local function mKW(n,l) if not n then return false end local ln=n:lower() for _,k in ipairs(l) do if ln:find(k,1,true) then return true end end return false end
local function gT(kw)
    local c = LP.Character
    if c then for _,t in ipairs(c:GetChildren()) do if t:IsA("Tool") and mKW(t.Name,kw) then return t end end end
    local bp = LP:FindFirstChild("Backpack")
    if bp then for _,t in ipairs(bp:GetChildren()) do if t:IsA("Tool") and mKW(t.Name,kw) then return t end end end
    return nil
end
local function eq(t) if not t then return false end local c=LP.Character if not c then return false end local h=c:FindFirstChildOfClass("Humanoid") if not h then return false end if t.Parent~=c then h:EquipTool(t) wait(0.15) end return true end
local function isMe(m) for _,p in ipairs(P:GetPlayers()) do if p.Character==m then return true end end return false end
local function gN(range)
    ...
    if gf then for _,fn in ipairs({"Customers","NPCs"}) do local f=gf:FindFirstChild(fn) if f then for _,m in ipairs(f:GetDescendants()) do if m:IsA("Model") and not seen[m] and not isMe(m) then local h=m:FindFirstChildOfClass("Humanoid") local mp=m:FindFirstChild("HumanoidRootPart") if h and mp and h.Health>0 then local d=(mp.Position-pos).Magnitude if d<=rr then seen[m]=true table.insert(npcs,{M=m,H=h,P=mp,D=d}) end end end end end end end
...
local function gNCached(range)
    ...
    if now-npcCacheTime<2 and #npcCache>0 then
        ...
        local pos=hrp.Position local rr=range+5 local result={}
        for _,t in ipairs(npcCache) do if t.P and (t.P.Position-pos).Magnitude<=rr then table.insert(result,t) end end
        return result
    end
    npcCache=gN(range) npcCacheTime=now
    return npcCache
end
...
local function iC(body) local n=body.Name:lower() for _,k in ipairs(BK) do if n:find(k,1,true) then return true end end for _,k in ipairs(NBK) do if n:find(k,1,true) then return false end end return n:find("body",1,true) or n:find("corpse",1,true) or n:find("dead",1,true) end
local function gB() local b={} for _,obj in ipairs(CS:GetTagged("Pickable")) do if obj:IsA("Model") and iC(obj) then table.insert(b,obj) end end return b end
local function gM()
    local bills={} local items=WS:FindFirstChild("ITEMS")
    if items then
        for _,obj in ipairs(items:GetDescendants()) do
            local pp=obj:FindFirstChildOfClass("ProximityPrompt")
            if pp then table.insert(bills,{M=obj,P=pp}) end
            if obj:IsA("BasePart") and mKW(obj.Name,{"cash","money","bill","coin"}) then if not pp then table.insert(bills,{T=obj}) end end
        end
    end
    return bills
end
local function fP(name) local wp=WS:FindFirstChild("WORLDPARTS") if not wp then return nil end for _,obj in ipairs(wp:GetDescendants()) do if obj:IsA("BasePart") and obj.Name==name then return obj end end return nil end
local function gPP(model)
    if not model then return nil end
    if model:IsA("BasePart") then return model end
    if model:IsA("Model") then local ok,pp=pcall(function() return model.PrimaryPart end) if ok and pp then return pp end end
...
local function fHP(model)
    ...
    for _,n in ipairs({"Head","UpperTorso","Torso","LowerTorso"}) do local p=model:FindFirstChild(n,true) if p and p:IsA("BasePart") then return p end end
    for _,d in ipairs(model:GetDescendants()) do if d:IsA("BasePart") and d.Name~="HumanoidRootPart" then return d end end
    return nil
end
...
local function dK()
    ...
    for _,t in ipairs(killTargets) do
        if t.P and t.P.Parent==t.M then
            ...
            if hNow and hNow.Health>0 and d<=rr then table.insert(alive,t) end
        end
    end
    killTargets=alive
    if #killTargets<5 then
        local existing={} for _,t in ipairs(killTargets) do existing[t.M]=true end
        local npcs=gNCached(S.KillRange)
        for _,npc in ipairs(npcs) do
            if #killTargets>=5 then break end
            if not existing[npc.M] then table.insert(killTargets,npc) existing[npc.M]=true end
        end
        if #killTargets>0 then print("[Aura] targets="..#killTargets) end
    end
    ...
    for _,t in ipairs(killTargets) do
        ...
        if hNow and hNow.Health>0 then
            local hp=fHP(t.M) if not hp then hp=t.P end
            for i=1,2 do MeleeEvent:FireServer(hp,hp.Position,NF,S.AkillDamage) wait(0.04) end
        end
    end
    local newList={}
    for _,t in ipairs(killTargets) do
        local hNow=t.M:FindFirstChildOfClass("Humanoid")
        if hNow and hNow.Health>0 then table.insert(newList,t) end
    end
    if #newList<#killTargets then print("[Aura] "..(#killTargets-#newList).." DEAD, "..#newList.." remain") end
    killTargets=newList
end
local function dG()
    local grinder=fP("Grinder") if not grinder then return end
    local bodies=gB() if #bodies==0 then return end
    local body=bodies[1]
    local c=LP.Character if not c then return end
    local hrp=c:FindFirstChild("HumanoidRootPart") if not hrp then return end
    ...
    if remotesReady and PickupEvent and DropEvent then
        ...
        if ok1 then print("[Grind] Pickup OK") else print("[Grind] Pickup ERR:"..tostring(err1)) return end
        wait(0.5)
        hrp.CFrame=grinder.CFrame*CFrame.new(0,0,2.5)
        wait(0.3)
        local ok2,err2=pcall(function() DropEvent:FireServer(body,grinder.Position) end)
        if ok2 then print("[Grind] Drop OK") else print("[Grind] Drop ERR:"..tostring(err2)) end
    end
end
local function dC()
    local c=LP.Character if not c then return end
    local hrp=c:FindFirstChild("HumanoidRootPart") if not hrp then return end
    local bills=gM() if #bills==0 then return end
    local n=0
    for _,b in ipairs(bills) do
        if b.P then
            local target=nil
            if b.M:IsA("BasePart") then target=b.M elseif b.M:IsA("Model") then for _,d in ipairs(b.M:GetDescendants()) do if d:IsA("BasePart") then target=d break end end end
            ...
            if d<=80 then
                ...
                wait(0.15) pcall(function() fireproximityprompt(b.P) end) n=n+1
            end
        elseif b.T then
            local d=(b.T.Position-hrp.Position).Magnitude
            if d<=80 then
                hrp.CFrame=b.T.CFrame*CFrame.new(0,2,0) wait(0.15)
                local pp=b.T:FindFirstChildOfClass("ProximityPrompt") if pp then pcall(function() fireproximityprompt(pp) end) end n=n+1
            end
        end
...
local function sP()
    ...
    PS={} wait(0.3)
    local sg=Instance.new("ScreenGui") sg.Name="BP" sg.ResetOnSpawn=false sg.DisplayOrder=999999 sg.IgnoreGuiInset=true sg.Parent=C
    ...
    for i=1,50 do
        ...
        d.Position=UDim2.new(0.2+math.random()*0.6,0,0.2+math.random()*0.6,0)
...
local function mE(t)
    ...
    local bb=Instance.new("BillboardGui") bb.Size=UDim2.new(0,200,0,50) bb.MaxDistance=S.EspRange bb.AlwaysOnTop=true bb.StudsOffset=Vector3.new(0,4,0)
    pcall(function() bb.Parent=t end)
    local tl=Instance.new("TextLabel") tl.Size=UDim2.new(1,0,1,0) tl.Text="NPC" tl.TextColor3=Color3.fromRGB(255,40,40) tl.BackgroundTransparency=0.7 tl.BackgroundColor3=Color3.new(0,0,0) tl.TextScaled=true tl.Font=Enum.Font.SourceSansBold tl.Parent=bb
    local hl=Instance.new("Highlight") hl.FillColor=Color3.fromRGB(255,40,40) hl.OutlineColor=Color3.fromRGB(255,255,255) hl.FillTransparency=0.3 hl.OutlineTransparency=0 hl.DepthMode=Enum.HighlightDepthMode.AlwaysOnTop
    pcall(function() hl.Parent=t end)
    EO[t]={B=bb,H=hl}
end
local function cE(t) local e=EO[t] if e then pcall(function() e.B:Destroy() end) pcall(function() e.H:Destroy() end) EO[t]=nil end end
local function cA() for t,_ in pairs(EO) do cE(t) end EO={} end
local function dE()
    if not S.EspEnabled then return end
    local c=LP.Character local hrp=c and c:FindFirstChild("HumanoidRootPart") local pos=hrp and hrp.Position
    for _,obj in ipairs(WS:GetDescendants()) do
        if obj:IsA("Humanoid") and obj.Parent and obj.Parent:IsA("Model") then
            ...
            if not isMe(m) then
                ...
                if d>S.EspRange then cE(m) elseif d<=S.EspRange and obj.Health>0 then mE(m) end
            end
        end
    end
end
...
local function mW()
    WN=WI:CreateWindow({Title="Burger",Author="bilibili",Icon="solar:hamburger-bold",Size=UDim2.fromOffset(750,520),ToggleKey=Enum.KeyCode.RightShift,Folder="burger-script",Acrylic=true,Resizable=false,ScrollBarEnabled=true,HideSearchBar=true,
        OnClose=function()
            xP() S.KillNPC=false S.GrindBodies=false S.CollectMoney=false S.AutoMode=false S.EspEnabled=false cA() killTargets={}
    ...
    t3:Dropdown({Flag="Theme",Title="Theme",Values={"Dark","Light","Rose","Plant","Ocean","Sunset","Midnight","Forest","Lavender","Coral","Mint","Sky","Blood","Lemon","Cyber"},Value="Dark",Callback=function(v) pcall(function() WI:SetTheme(v) end) S.ParticleColor=tc(v) end})
    local t4=WN:Tab({Title="Stats",Icon="solar:chart-bold"})
    local npcP=t4:Paragraph({Title="NPC:0"}) local bodyP=t4:Paragraph({Title="Bodies:0"}) local moneyP=t4:Paragraph({Title="Money:0"})
    ...
    pcall(function()
        ...
        t5:Space()
        local AC={} pcall(function() AC=CM:AllConfigs() end) local DV=nil pcall(function() for _,v in ipairs(AC) do if v=="default" then DV="default" break end end end)
    ...
    t6:Paragraph({Title="Burger Script v2.22"}) t6:Divider()
    t6:Paragraph({Title="Author",Desc="bilibili"})
    t6:Paragraph({Title="v2.22",Desc="Multi-target aura"})
...
local PP=false
WI:Popup({Title="Burger v2.22",Content="Multi-target aura",Buttons={{Title="Load",Callback=function() PP=true end,Variant="Primary"}}})
while not PP do wait(0.1) end
spawn(function()
    local npcP,bodyP,moneyP=mW()
    print("[v2.22] loop start")
    local last=0
    while true do
        if S.KillNPC then pcall(function() dK() end) wait(0.15) end
        if S.GrindBodies then pcall(function() dG() end) wait(0.2) end
        if S.CollectMoney then pcall(function() dC() end) wait(0.5) end
        pcall(function() dE() end)
        local now=tick()
        if now-last>3 then
            last=now npcCacheTime=0
